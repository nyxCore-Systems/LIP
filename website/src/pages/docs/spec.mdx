...
- [x] **`IndexChanged` push notification** — emitted to all active sessions after every successful `Delta::Upsert`. Carries `indexed_files` count and `affected_uris`. Enables precise cache invalidation without polling `QueryIndexStatus`.

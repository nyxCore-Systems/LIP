//! gopls Tier 2 backend for Go files.
//!
//! Spawns `gopls` as a child process (requires gopls in PATH — install with
//! `go install golang.org/x/tools/gopls@latest`) and communicates over the
...
//!
//! If `gopls` is not in PATH the backend fails to spawn and is permanently
...
impl GoplsBackend {
    /// Spawn `gopls`, initialize the LSP session, and return a ready backend.
    ///
    /// `workspace_root` is passed as `rootUri` so gopls can locate `go.mod`.
    /// Pass `None` for single-file / GOPATH mode.
    ///
    /// Returns `Err` if `gopls` is not found in PATH.
    pub async fn new(workspace_root: Option<PathBuf>) -> anyhow::Result<Self> {
        let mut child = Command::new("gopls")
            ...
            .spawn()
            .context(
                "gopls not found — install it with `go install golang.org/x/tools/gopls@latest` \
    ...
    async fn hover_signature(
        ...
        // gopls returns hover as { contents: { kind: "markdown", value: "..." } }
        let md = result
        ...
        Ok(extract_go_signature(md))
    }

    // ── Public: full verification ─────────────────────────────────────────────

    /// Run Tier 2 verification on a single Go file.
    pub async fn verify_file(
        ...
        debug!("tier2(gopls): {} raw symbols for {uri}", raw.len());
...
/// Extract a Go type signature from gopls hover markdown.
///
/// gopls wraps signatures in a fenced `go` code block.
fn extract_go_signature(md: &str) -> Option<String> {
...
fn lsp_kind_to_lip(kind: u64) -> SymbolKind {
    match kind {
        ...
        3 => SymbolKind::Namespace, // Namespace / Package
        5 => SymbolKind::Class,
        6 => SymbolKind::Method,
        7 => SymbolKind::Field,

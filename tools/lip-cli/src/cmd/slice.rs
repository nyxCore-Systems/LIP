...
async fn slice_cargo(manifest: &Path, output: &Path, args: &SliceArgs) -> anyhow::Result<usize> {
    ...
    let deps: Vec<&CargoPackage> = meta.packages.iter()
    ...
    for pkg in deps {
        ...
        println!("  {}@{}  ({} symbols)  {n}", pkg.name, pkg.version, slice.symbols.len());
        count += 1;
    }
...
async fn slice_npm(manifest: &Path, output: &Path, args: &SliceArgs) -> anyhow::Result<usize> {
    ...
    let node_modules = manifest.parent().unwrap_or(Path::new("."))
        .join("node_modules");

    anyhow::ensure!(node_modules.exists(),
        "node_modules not found at {} — run `npm install` first",
        node_modules.display());

    let mut count = 0usize;
    let all_deps: std::collections::HashMap<&str, &str> = pkg.dependencies.iter()
    ...
    for (name, version_spec) in all_deps {
        ...
        println!("  {name}@{version}  ({} symbols)  {hash}", slice.symbols.len());
        count += 1;
    }
...
async fn slice_pub(manifest: &Path, output: &Path, args: &SliceArgs) -> anyhow::Result<usize> {
    ...
    let lock_path = manifest.parent().unwrap_or(Path::new("."))
        .join("pubspec.lock");
...
fn index_directory(dir: &Path, pkg_name: &str, version: &str, manager: &str) -> Vec<OwnedSymbolInfo> {
    ...
    {
        let path = entry.path();
        let uri  = format!("lip://{scope}/{pkg_name}@{version}/{}",
...
async fn push_slice(slice: &OwnedDependencySlice, _hash: &str, registry: &str) -> anyhow::Result<()> {
